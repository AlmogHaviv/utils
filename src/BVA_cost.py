...
def divide_by_months(df):
    ...
    df['Date'] = pd.to_datetime(df['Latest Sprint Date'], infer_datetime_format=True)

    # Create a new column 'Month' to store the month from the 'Date' column
    df['Month'] = df['Sprint'].dt.month
...
def create_cost_per_days_df(df):
    ...
    multipliers = {
        ...
        'Devops': 1061,
        'Dev': 1059,
        'System Architect': 1140
    }

    # Get all column names starting from the third column
    columns_to_multiply = df.columns[2:]

    # Apply the multiplication based on the Team Name
    df[columns_to_multiply] = df.apply(
        lambda row: row[columns_to_multiply] * multipliers.get(row['Team Name'], 1), axis=1)
    return df


def export_dataframe_to_excel_with_merged(df, output_filename):
    cost_df = create_cost_per_days_df(df)
    ...
    cost_df['Company Name'] = cost_df['Company Name'].apply(remove_prefix)

    # Create a new Excel writer object
    writer = pd.ExcelWriter(output_file_path, engine='openpyxl')
    cost_df.to_excel(writer, index=False, sheet_name='Sheet1')
